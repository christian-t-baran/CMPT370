 ...
 */
public class TestBenchPanel extends JPanel {
    ...
    /**
     * Paints the results on the main panel
     * @param c Controller to get game info from
     */
    ...
    private void buttons(ActionListener listener) {
        ...
        stepBack.setBorderPainted(false);
        ...
        try {
            ...
            stop.setIcon(new ImageIcon(stopImg.getScaledInstance(IMG_WIDTH, IMG_HEIGHT,
                    Image.SCALE_DEFAULT)));
            Image stepBackImg = ImageIO.read(getClass().getResource("/imgs/rewind.png"));
            stepBack.setIcon(new ImageIcon(stepBackImg.getScaledInstance(IMG_WIDTH, IMG_HEIGHT,
                    Image.SCALE_DEFAULT)));
        } catch (IOException e) {
        }
        ...
        stepBack.setPreferredSize(new Dimension(40, 40));
        stepBack.addActionListener(listener);
        stepBack.setActionCommand("stepback");
     ...
     */
    public TestBenchPanel(int width, int height, ActionListener listener, int boardSize, Controller controller) {
